...
VisualOdometryMonoNew::VisualOdometryMonoNew (parameters param) : VisualOdometry((VisualOdometry::parameters)param), param(param) {
}

VisualOdometryMonoNew::~VisualOdometryMonoNew () {
}

bool VisualOdometryMonoNew::process (uint8_t *I,int32_t* dims,bool replace) {
  matcher->pushBack(I,dims,replace);
  matcher->matchFeatures(0);
  matcher->bucketFeatures(param.bucket.max_features,param.bucket.bucket_width,param.bucket.bucket_height);                          
  p_matched = matcher->getMatches();
  return VisualOdometry::updateMotion();
}


vector<double> VisualOdometryMonoNew::estimateMotion (vector<Matcher::p_match> p_matched) {
...
Matrix VisualOdometryMonoNew::smallerThanMedian (Matrix &X,double &median) {
...
bool VisualOdometryMonoNew::normalizeFeaturePoints(vector<Matcher::p_match> &p_matched,Matrix &Tp,Matrix &Tc) {
...
void VisualOdometryMonoNew::fundamentalMatrix (const vector<Matcher::p_match> &p_matched,const vector<int32_t> &active,Matrix &F) {
...
vector<int32_t> VisualOdometryMonoNew::getInlier (vector<Matcher::p_match> &p_matched,Matrix &F) {
...
void VisualOdometryMonoNew::EtoRt(Matrix &E,Matrix &K,vector<Matcher::p_match> &p_matched,Matrix &X,Matrix &R,Matrix &t) {
...
int32_t VisualOdometryMonoNew::triangulateChieral (vector<Matcher::p_match> &p_matched,Matrix &K,Matrix &R,Matrix &t,Matrix &X) {
...
///TODO: NEW
//
bool VisualOdometryMonoNew::process (uint8_t *I,int32_t* dims, Matrix n, double h, bool replace) {

  matcher->pushBack(I,dims,replace);
  matcher->matchFeatures(0);
  matcher->bucketFeatures(param.bucket.max_features,param.bucket.bucket_width,param.bucket.bucket_height);
  p_matched = matcher->getMatches();
  return updateMotion(n,h);
}

bool VisualOdometryMonoNew::process (uint8_t *I,int32_t* dims, Matrix n, double h, const std::unique_ptr<CameraModel> &CamModel, bool replace){
	matcher->pushBack(I,dims,replace);
	matcher->matchFeatures(0);
	matcher->bucketFeatures(param.bucket.max_features,param.bucket.bucket_width,param.bucket.bucket_height);
	p_matched = matcher->getMatches();
	return updateMotion(n,h,CamModel);
}


bool VisualOdometryMonoNew::process_ohne_skale (uint8_t *I,int32_t* dims, bool replace) {

  matcher->pushBack(I,dims,replace);
  matcher->matchFeatures(0);
  matcher->bucketFeatures(param.bucket.max_features,param.bucket.bucket_width,param.bucket.bucket_height);
  p_matched = matcher->getMatches();
  return updateMotion_ohne_skale();
}

bool VisualOdometryMonoNew::process_with_Groundplane_estimation (uint8_t *I,int32_t* dims, double h, bool replace) {

  matcher->pushBack(I,dims,replace);
  matcher->matchFeatures(0);
  matcher->bucketFeatures(param.bucket.max_features,param.bucket.bucket_width,param.bucket.bucket_height);
  p_matched = matcher->getMatches();
  return updateMotion_with_Groundplane_estimation(h);
}


bool VisualOdometryMonoNew::updateMotion (viso2::Matrix n, double h) {

  // estimate motion
  vector<double> tr_delta = estimateMotion(p_matched,n , h);
...
bool VisualOdometryMonoNew::updateMotion (viso2::Matrix n, double h, const std::unique_ptr<CameraModel> &CamModel){

	// estimate motion
	vector<double> tr_delta = estimateMotion(p_matched,n , h, CamModel);
...
bool VisualOdometryMonoNew::updateMotion_ohne_skale () {

  // estimate motion
  vector<double> tr_delta = estimateMotion_ohne_skale(p_matched);
...
bool VisualOdometryMonoNew::updateMotion_with_Groundplane_estimation (double h) {

  // estimate motion
  vector<double> tr_delta = estimateMotion_with_Groundplane_estimation(p_matched, h);
...
vector<double> VisualOdometryMonoNew::estimateMotion (vector<Matcher::p_match> p_matched, viso2::Matrix n, double h) {
  ...
  Matrix   d        = ~n*X_plane.getMat(0,0,2,-1);
  double   sigma    = median/50.0;
  double   weight   = 1.0/(2.0*sigma*sigma);
  double   best_sum = 0;
  int32_t  best_idx = 0;
...
std::vector<double>  VisualOdometryMonoNew::estimateMotion (std::vector<Matcher::p_match> p_matched ,viso2::Matrix n, double h, const std::unique_ptr<CameraModel> &CamModel){
...
vector<double> VisualOdometryMonoNew::estimateMotion_ohne_skale (vector<Matcher::p_match> p_matched) {
...
std::vector<double> VisualOdometryMonoNew::scale_pose(viso2::Matrix n, double h){
...
std::vector<double> VisualOdometryMonoNew::estimateMotion_with_Groundplane_estimation (std::vector<Matcher::p_match> p_matched, double h) {
...
//
//  //RANSAC
//  std::vector<double> ebenen_parameter;
//  int32_t Ne = X_plane.n;
//  double* Xe = new double[Ne];
//  double* Ye = new double[Ne];
//  double* Ze = new double[Ne];
//
//  for (int32_t i=0; i<Ne; i++){
//	  Xe[i] = X_plane.val[0][i];
//	  Ye[i] = X_plane.val[1][i];
//	  Ze[i] = X_plane.val[2][i];
//  }
//
//  TofGround::parameters paramet;
//  paramet.ransac_iter = 200;
//  paramet.inlier_threshold = 0.1;
//  TofGround togr(paramet,10,10);
//  std::vector<int32_t> inliers;
//  TofGround::planeHN cur_plane;
//  TofGround::planeHN plane;
//
//  for (int32_t i=0; i<paramet.ransac_iter; i++){
//	  std::vector<int32_t> active = getRandomSample(Ne, 3);
//	  cur_plane = togr.getplane(active, Xe, Ye, Ze);
//	  std::vector<int32_t> inliers_cur;
//	  inliers_cur = togr.getInlier(cur_plane, Xe, Ye, Ze, Ne);
//
//	  if(inliers_cur.size() > inliers.size()){
//		  inliers = inliers_cur;
//		  plane = cur_plane;
//	  }
//  }
//
//  alle_pkt_ebene = inliers;
//  delete[] Xe;
//  delete[] Ye;
//  delete[] Ze;
//
